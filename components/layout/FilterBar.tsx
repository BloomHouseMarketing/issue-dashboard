...
export default function FilterBar() {
  const { filters, filterOptions, setFilter, toggleIssueType, resetFilters, hasActiveFilters } = useFilters();
      ...
      {hasActiveFilters && (
        <div className="flex flex-wrap gap-2 mt-2">
          ...
          {filters.state && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#3B82F6]/10 text-[#3B82F6] text-xs rounded-full">
              {filters.state}
              <button onClick={() => setFilter('state', null)} className="hover:text-[#2563EB] ml-0.5">&times;</button>
            </span>
          )}
          {filters.shift && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#3B82F6]/10 text-[#3B82F6] text-xs rounded-full">
